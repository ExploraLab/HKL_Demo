...
def runHKTAnalysis(input_file, output_file, file_type):
    # Endpoint URL
    endpoint = r"https://fr-hkl-demo.cognitiveservices.azure.com/"
    # Subscription Key
    apim_key = "193affdd3a1f43958c60674a4421ec75"
    # Model ID
    model_id = "af643ba0-645a-4c48-b84f-4518067ce8a3"
    # API version
    ...
    while n_try < n_tries:
        try:
            ...
            if status == "succeeded":
                ...
                for invoice in enumerate(data['analyzeResult']['documentResults']):
                    vendorName = invoice[1]['fields']['VendorName']['valueString']
                    BeneficiaryName = vendorName
                    invoiceID = invoice[1]['fields']['InvoiceID']['valueString']
                    invoiceDate = invoice[1]['fields']['InvoiceDate']['valueDate']
                    invoiceTotal = invoice[1]['fields']['InvoiceTotal']['valueString']
                    customerID = invoice[1]['fields']['CustomerID']['valueString']

                pageResultSetStr = {'Page': 1, 'CompanyName': vendorName, 'BeneficiaryName': BeneficiaryName,
                                    'InvoiceID': invoiceID,
